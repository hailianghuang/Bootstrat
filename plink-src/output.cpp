...
void Plink::display_recoded_VCF()
{
  
  string f = par::output_file_name + ".vcf";
  
  printLOG("Writing recoded VCF file to [ " + f + " ] \n");
  ofstream VCF(f.c_str(), ios::out);
  VCF.clear();
  ...
  if ( par::output_only_fid ) 
    for (int i=0;i<n;i++) VCF << "\t" << sample[i]->fid;
  else if ( par::output_only_iid ) 
    for (int i=0;i<n;i++) VCF << "\t" << sample[i]->iid;
  else
    for (int i=0;i<n;i++) VCF << "\t" << sample[i]->fid << "_" << sample[i]->iid;
  ...
  for (int l=0;l<nl_all;l++)
    {
      ...
      std::string alt = locus[l]->allele1 == "0" ? "." : locus[l]->allele1 ;
...
void Plink::output_beagle_format()
{

  string f = par::output_file_name + ".dat";
  vector<int> r = getChromosomeRange(*this);
      
  printLOG("\nWriting per-chromosome BEAGLE files to [ "
	   + par::output_file_name + ".chr-*.dat ]\n");
  printLOG("and marker information to [ "
	   + par::output_file_name + ".chr-*.map ]\n\n");

  // Only write autosomes (for humans)
  ...
  for (int c = r[0] ; c <= r[1] ; c++)
    {
      ...
      printLOG("Writing chromosome " + int2str(c) + "\n");
      string f = par::output_file_name + ".chr-" + int2str(c);
      ofstream GEN((f+".dat").c_str(), ios::out); 
      ofstream MAP((f+".map").c_str(), ios::out); 
	  ...
	  if ( locus[l]->chr == c )
	    {
	      ...
	      if ( locus[l]->allele1 == "0" ) 
		MAP << "X\t";
	      else
		MAP << locus[l]->allele1 << "\t";
	      
	      if ( locus[l]->allele2 == "0" ) 
		MAP << "X\n";
	      else
		MAP << locus[l]->allele2 << "\n";
	    }
