  ...
  for (int k=0; k<ns; k++)
    {
	  ...
	  if((par::perm_prob_file).length() > 1){
	/*
			FILE *fp = fopen("temp.txt", "w");	
			for (int j=0; j<P.sample.size(); j++){
				fprintf(fp, "%s %s\n", (P.sample[j]->fid).c_str(), (P.sample[j]->iid).c_str());
			}
			fclose(fp);
	*/	if(par::usePerm){
			permuteProb(p);
...
//			printf("%d, %d, %d, %d, %d\n", j, k, (int)i[j][k], s[j][k],s[j][(int)i[j][k]] );
     	P.sample[s[j][k]]->pperson = P.sample[s[j][(int)i[j][k]]];
//		P.sample[s[j][k]]->pperson->aff= (CRandom::rand() < 0.5);
...
bool Perm::update(vector<double> & result, vector<double> & original)
{
  ...
  if (!adaptive)
    {
      for (int l=0; l<t; l++)
	if ( compare( result[l] , original[l] ) ) R[l]++;
    }
  else
    {
      for (int l=0; l<t; l++)
...
void Perm::nextSet()
{
  // Reset Perm class for next set
  performed = 0;
  R_GWiS=0;
  originalOrder();
}

bool Perm::updateGWiS(double result, double original){
  
  // Increment number of permutations performed for this gene
  performed++;

  // Finished all perms for this gene?
  bool done = false;
  
  //////////////////////////////
  // Update number of successes

  if (result >= original  || !realnum(original) ) 
    R_GWiS++;
  ...
  if (adaptive && 
      ...
      if (R_GWiS>0)
	{
	  ...
	  double pv = (double)(R_GWiS+1)/(double)(performed+1);
...
double Perm::GWiS_pvalue()
{
  return (double)(R_GWiS+1) / (double)(performed+1);
}
