...
//   display(eigenvector);
  ...
  // Sqrt(D)
  vector_t sqrt_eigenvalue(nc);
  
  for (int i=0; i<nc; i++) 
    sqrt_eigenvalue[i] = eigenvalue[i] >= 0 ? sqrt(eigenvalue[i]) : 0 ;
  ...
  for (int c1=0; c1<nc; c1++)
      ...
      for (int c3=0; c3<par::cluster_mds_dim; c3++)
	{
	  ...
	  int i3 = elist[c3];
	  if ( i3 == i2 )
 	    mds[c1][c2] += eigenvector[c1][i3] * sqrt_eigenvalue[i2];
	}


  ////////////////////////////////////////////////////////
  // Give indication for number of components to select

//   // k is set 0<k<1;  default = 0.7
  
//   double k1 = 0.5;
//   double k2 = 0.7;
//   double k3 = 0.9;
  
//   sort(eigenvalue.begin(), eigenvalue.end(), std::greater<double>() );

//   double s2 = 0;

//   for(int e=0; e<eigenvalue.size(); e++)
//     s2 += eigenvalue[e];
...
//   //  cout << nc << " and " << eigenvalue.size() << "\n";

//   double thresh = 0.7 / (double)nc;
//   double totvar = 0;
  
//   for ( int i=0; i<nc; i++)
//     totvar += eigenvalue[i] * eigenvalue[i];

//   int kept = -1;
//   for ( int i=0; i<nc; i++)
//     {
//       if ( ( eigenvalue[i] * eigenvalue[i] ) / totvar >= thresh )
// 	{
// 	  kept = i+1;
// 	}
//     }
...
//       // 50%, 70% and 90% thresholds; also Dunn & Everitt rule
//       if ( s1 < s2*k1 && s2*k1 < s3 ) cout << "1 "; else cout << "0 ";
//       if ( s1 < s2*k2 && s2*k2 < s3 ) cout << "1 "; else cout << "0 ";
//       if ( s1 < s2*k3 && s2*k3 < s3 ) cout << "1 "; else cout << "0 ";
//       if ( kept == r ) cout << "1 "; else cout << "0 ";
