...
{  
  ...
  // Set all non-zero eigenvalues to 1
  matrix_t pcM;
  sizeMatrix(pcM,ntot,ntot);
  ...
  matrix_t t1;
  matrix_t t2;
  matrix_t pvT(ntot,ntot);
  for (int i=0; i<ntot; i++)
    for (int j=0; j<ntot; j++)
      pvT[j][i] = pv[i][j];

  multMatrix( ps , pcM , t1 );
  multMatrix( t1 , pvT , t2 );
...
//   cout << "and T2:\n";
//   display(t2);
//   cout << "from " << g[0].size() << " to " << t2[0].size() << "\n";
  
  // t2 is now a N x P matrix -- sum these
  ...
  for (int i=0; i<P->n; i++)
      ...
      for (int p = 0 ; p < ntot ; p++ )
	{
      ...
      P->sample[i]->clist[ par::clist_number - 1 ] = sc;
    }

  // Track number of PCs so they can be accounted for in the next analysus
  //  npc = pcn;
      ...
      for ( int bp = bpstart; bp <= bpstop; bp += par::rarer_interval )
	{
	  ...
	  if ( disp )
	    {      
	      ...
	      int bp1 = bp - (int)par::rarer_dist_threshold < bpstart ? bpstart : bp - (int)par::rarer_dist_threshold;
	      int bp2 = bp + (int)par::rarer_dist_threshold > bpstop ? bpstop   : bp + (int)par::rarer_dist_threshold;
	      ...
	      if ( par::rare_test_print_details && 
		   int2str(chr)+":"+int2str(bp1) == par::rare_test_print_details_snp )
		{
