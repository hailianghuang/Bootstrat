...
void permuteProb( vector<long int> & a )
{
	static double *prob=NULL;
	static char *ptr, *next, sline[1000000];
	int i, j, N;
	double r;
	i=0;
	N = a.size();
	...
	if(prob==NULL){
		printf("Start to read prob file (permutation): %s\n", (par::perm_prob_file).c_str());
		printf("%d samples to read\n", N);
		prob = (double*) malloc(N*N*sizeof(double));
		FILE * fp = fopen((par::perm_prob_file).c_str(), "r");
		while(fgets(sline, 1000000, fp)){ 
			ptr = sline;
			for (int j=0; j<N; j++){
				next = strchr(ptr, '\t');
				if(next !=NULL) *next='\0';
  ...
  for( long int i = N-1; i > 0;  i-- )
    {
      double r = CRandom::rand() * prob[i+i*N];
...
void boot( vector<long int> & a )
{
	...
	i=0;
	N = a.size();
	static int N_jump=(N-1)*N;
	
	if(prob==NULL){
		printf("Start to read prob file (bootstrap): %s\n", (par::perm_prob_file).c_str());
		printf("%d samples to read\n", N);
		prob = (double*) malloc(N*N*sizeof(double));
		FILE * fp = fopen((par::perm_prob_file).c_str(), "r");
		while(fgets(sline, 1000000, fp)){ 
  ...
  for( long int i = N-1; i >= 0;  i-- )
    {
      double r = CRandom::rand() * prob[i+N_jump];
	  for(j=0; j<N; j++)
		if(r <= prob[i+j*N])
			break;
	  a[i]=j;
...
void Plink::attribAssignSNP()
{

  attribs = true;

  printLOG("Loading attributes for markers, from [ " + par::snp_attrib_file + " ]\n");
  
  checkFileExists( par::snp_assign_file );  
  ifstream IN1( par::snp_assign_file.c_str() , ios::in );
  ...
  while ( ! IN1.eof() )
	...
	continue;
      ...
      for (int j=1; j<tok.size(); j++)
	str += tok[j] + "|";      
      attrib.insert(make_pair( i->second , str ));
    }
