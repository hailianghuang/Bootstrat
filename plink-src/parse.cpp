...
void setOptions(CArgs & a)
{
  ...
  if (a.find("--lookup"))
   ...
   if (a.find("--recode-vcf")) 
     {
       ...
       if ( a.find("--iid") ) par::output_only_iid = true;
       else if ( a.find("--fid") ) par::output_only_iid = true;
   ...
   if (a.find("--recode-beagle"))
     {
       makedata = true;
       par::recode_beagle = true;
       // and unless otherwise specified, set GENO = 1 and MAF = 0
       if (!a.find("--maf")) par::min_af = 0.0;
       if (!a.find("--geno")) par::MAX_GENO_MISSING = 1;     
       if (!a.find("--mind")) par::MAX_IND_MISSING = 1;
  ...
  if ( a.find("--lf-scan") )
    {
      par::lf_search = true;
      par::lf_search_snp = a.value("--lf-scan");
    }
...
// 	 error("Must specify --segment-group for --segment-spanning\n");
  ...
  if ( a.find("--vt-test") || a.find("--fw-test") || a.find("--rv-test") )
    {
      
      par::rare_test = true;
      
      if ( a.find("--vt-test") ) par::rtest_type = RTEST_VARIABLE_THRESHOLD;
      else if ( a.find("--fw-test") ) par::rtest_type = RTEST_FREQWEIGHT;
      else if ( a.find("--rv-test") ) 
	{
	  par::rtest_type = RTEST_VANILLA;
	  par::rtest_freq_threshold = a.value_double("--rv-test");
	}

      if ( a.find("--weights") )
	{
	  par::rtest_weight_file = a.value("--weights");
	  par::rtest_weights = true;
	}
      ...
      if ( a.find("--weights") )
	{
	  par::rtest_weight_file = a.value("--weights");
	  par::rtest_weights = true;
	  par::rtest_ppweights = true;
	}

      if ( a.find("--weight-threshold") )
	{
	  par::rtest_weight_threshold = a.value_double("--weight-threshold");
	}
      
      if ( a.find("--vt-test-2sided") )
	{
	  par::rtest_hyp = RTEST_2SIDED;	  
	}
      
      if ( a.find("--vt-test-low"))
	{
	  par::rtest_hyp = RTEST_1SIDED_LOW;	  
	}
  ...
  if (a.find("--D") || a.find("--dprime") ) 
    {
      par::disp_r2 = true;
      par::dprime = true;
      if ( ! ( a.find("--matrix") || a.find("--inter-chr") ) )
	par::disp_r_window = true;
      if ( a.find("--with-freqs") )
	par::ld_and_freqs = true;
    }
  ...
  if (a.find("--GWiS"))
    {
      ...
      // a set file has to be specified
      if (!a.find("--set"))
	error ("You need to specify --set also");

      if ( !a.find("--set-GWiS-SampleSize") )
	par::GWiS_SampleSize=1;
      
      if ( a.find("--gene") )
	error("GWiS does not currently work with --gene");
  ...
  if ( a.find("--set-GWiS-SampleSize") ){
    string GWiS_missing_method= a.value("--set-GWiS-SampleSize");
    if(GWiS_missing_method.compare("snp_pair")==0){
      par::GWiS_SampleSize=0;
    }else if(GWiS_missing_method.compare("snp")==0){
      par::GWiS_SampleSize=1;
    }else if (GWiS_missing_method.compare("full")==0) {
      par::GWiS_SampleSize=2;
    }else{
      error("Unknown parameter in --set-GWiS-SampleSize.  Choose from full, snp, and snp_pair");
    }
  }
...
//   if (a.find("--1aff")) { par::remove_unaffected_pairs = true; }
  ...
  if (a.find("--attrib")) 
    {
      par::snp_attrib_assign = true;
      par::snp_assign_file = a.value("--attrib");
    }

  if (a.find("--filter-attrib")) 
    {
      par::snp_attrib_filter = true;
      vector<string> s = a.value("--filter-attrib",2);
      par::snp_attrib_file = s[0];
      par::snp_attrib_value = s[1];
    }
  
  if (a.find("--filter-attrib-indiv")) 
    {
      par::ind_attrib_filter = true;
      vector<string> s = a.value("--filter-attrib-indiv",2);
  ...
  if (a.find("--hapfile")) 
    { 
      ...
      if (a.find("--map")) 
	par::mapfile = a.value("--map");
      else
	error("Need to specify a --map file");
      
      if ( a.find("--fam"))
	par::famfile = a.value("--fam");
      else
	error("Need to specify a --fam file");

      par::read_hapfile = true;
      par::hapfilename = a.value("--hapfile");
      
      if ( a.find("--hap-coding"))
	par::hapfile_coding = a.value_int("--hap-coding");
  ...
  if ( a.find("--hfile") )
    {
      ...
      par::read_hapfile = true;
      string rootname = a.value("--hfile");
      par::mapfile = rootname + ".map";
      par::famfile = rootname + ".fam";
      par::hapfilename = rootname + ".haps";
      if ( a.find("--hap-coding"))
	par::hapfile_coding = a.value_int("--hap-coding");
    }
  ...
  if ( a.find("--pfile") )
    {
      par::pool_input = true;
      string rootname = a.value("--pfile");
      par::mapfile = rootname + ".map";
      par::famfile = rootname + ".pdef";
      par::poolfile = rootname + ".pgen";      
    }
  ...
  if (a.find("--perm"))
	...
	if (a.find("--Pperm")) 
  {
      par::perm_prob_file = a.value("--Pperm");
  }

	if(a.find("--print_shuffle")){
		par::print_shuffle_file = a.value("--print_shuffle");
	}
  ...
  if (a.find("--aperm"))
	...
	error("--segment options requires --pperm option");
      ...
      //GWiS works with adaptive permutations
      if ((a.find("--set")||a.find("--make-set")) && !a.find("--GWiS"))
