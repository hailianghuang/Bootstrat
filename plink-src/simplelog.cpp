...
// Fit logistic model              lm.fitLogM();


vector_t Plink::logisticAssoc(bool print_results, Perm & perm)
{
  ...
  if (print_results)  
    {
      string f = par::output_file_name + ".assoc.logit";
      //if (par::twoDFmodel) f += ".genotypic";
      printLOG("Writing logistic model association results to [ " + f + " ] \n");
  ...
  while ( s != SNP.end() )
	   ...
	   par::chr_haploid[locus[l]->chr] )
	{
      ...
      // Fit model (sets dependent)
      LogisticModel lm(this);
	...
	lm.displayResults(ASC,locus[l]);
       ...
       if (par::twoDFmodel)
 	{
	  ...
	  // test of whether same ( sign issue? )
