...
// Begin permutations
	...
	vector<double> perm_dump;
	int perm_dump_total=0;
	FILE * fp_shuffle;
	if((par::print_shuffle_file).length() > 1)
		fp_shuffle = fopen((par::print_shuffle_file).c_str(), "w");
...
//			printLOG("Perm started here\n");
		...
		if((par::print_shuffle_file).length() > 1){
			...
			while ( gperson !=  sample.end() ){
				...
				if (!pperson->missing){
					if(par::qt )
						fprintf(fp_shuffle, "%g\t", pperson->phenotype );
					else if(par::bt){
						if(pperson->aff)
							fprintf(fp_shuffle, "%d\t", 2 );
						else
							fprintf(fp_shuffle, "%d\t", 1 );
					}
				}
				gperson++;
			}
	...
	// Standard permutation counting
			...
			if(SAVE_ALL_TEST_STAT){
				for (int i=0; i<pr.size();i++){
					pr[i]= pr[i]!=pr[i] ? -1: pr[i];
					perm_dump.push_back(pr[i]);
				}
			}	
			perm_dump_total++;
		...
		if(SAVE_ALL_TEST_STAT){
			std::sort (perm_dump.begin(), perm_dump.end()); 
		}
		printLOG("Total number of permutations performed is " + int2str(perm_dump_total) + " \n");
...
/////////////////////////////////////////////////////
		...
		ofstream ASC, ASC_perm;
		string f, f_perm;

		if (par::adaptive_perm) f = par::output_file_name + f0 + ".perm";
		else f = par::output_file_name + f0 + ".mperm";

		f_perm = par::output_file_name + f0 + ".mperm.null";
		ASC_perm.open(f_perm.c_str(), ios::out);
		...
		if ( !par::set_r2 ) 
		{
			if (par::adaptive_perm)
				...
				ASC	<< setw(12)<< "EMP2" << " ";
				ASC	<< setw(12)<< "EMP_NULL" << " ";
				ASC	<< setw(12)<< "EMP_NULL_P" << " ";
			}
		}
		else 
			ASC << "SNPS";
	...
	// ... except 2df genotypic test
			...
			if ( ! par::set_r2 )
			{
				if (par::adaptive_perm) 
					...
					ASC << setw(12) << perm.max_pvalue(l) << " ";
					if(SAVE_ALL_TEST_STAT){
						double temp=perm_dump[l*perm_dump_total+perm_dump_total/2];
						if(temp < -0.5){
							ASC_perm << setw(12) << "NA" << " " ;
							ASC_perm << setw(12) << "NA" << " "<< "\n";
						}else{
							ASC_perm << setw(12) << temp << " ";
							ASC_perm << setw(12) << chiprobP(temp, 1) << " "<< "\n";
						}
					}
				}
			}
...
// Iterate over SNPs
	...
	// Adjustment for case-only test -- i.e. only consider instances where
	// we do not see the minor allele in controls

			if ( par::case_uniq && U1 > 0 ) original[l] = 0;
...
// one matrix, otherwise use  
		...
		if (!par::matrix)
		{
			...
			if ( par::ld_and_freqs ) LD << setw(10) << "MAF_A" << " ";      
			LD << setw(6) << "CHR_B"  << " " 
				<< setw(12) << "BP_B" << " "
			...
			if ( par::ld_and_freqs ) LD << setw(10) << "MAF_B" << " ";      

			if (par::disp_r1)
				LD << setw(12) << "R" << " " ;
			else
				LD << setw(12) << "R2" << " " ;
			if ( par::dprime) LD << setw(12) << "DP" << " ";
			LD << "\n";
		}
	...
	// Get R^2 
		...
		// Using a r^2 threshold? 
						...
						r >= par::disp_r_window_r2 )
					{
						...
						if ( par::ld_and_freqs ) LD << setw(10) << locus[l1]->freq << " ";      		  
						LD << setw(6) << locus[l2]->chr  << " " 
							<< setw(12) << locus[l2]->bp << " "
							<< setw(par::pp_maxsnp) << locus[l2]->name << " ";

						if ( coupling ) 
							LD << setw(10) << ( locus[l1]->allele1 + locus[l2]->allele1 
							+ "/" +  locus[l1]->allele2 + locus[l2]->allele2 ) << " ";
						else
							LD << setw(10) << ( locus[l1]->allele1 + locus[l2]->allele2 
							+ "/" +  locus[l1]->allele2 + locus[l2]->allele1 ) << " ";
