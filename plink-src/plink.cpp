...
int main(int argc, char* argv[]) 
{
  ...
  PVERSION = "1.08";        // 4 chars
  PREL = "p";               // space or p (full, or prelease) 
  PDATE    = "14/Aug/2009"; // 11 chars
  ...
  else if (par::pool_input) P.readDataPooledFormat();
  else if (par::lfile_input) P.readDataLongFormat();
  else if (par::read_hapfile) P.readHaplotypeFormat();
  ...
  /////////////////////////////////
  // Assign arbitrary SNP attributes

  if ( par::snp_attrib_assign )
    P.attribAssignSNP();
...
//     par::missing_phenotype = "0";
  ...
  if ( par::recode_beagle )
  {
    P.output_beagle_format();
    shutdown();
  }

  if ( par::recode_vcf )
  {
    P.display_recoded_VCF();
    shutdown();
  }
  ...
  ///////////////////////////////////////////
  // Attempt to split a common-allele 
  // associaiton with a lower-freq 
  // haplotype?

  if ( par::lf_search )
    {
      P.lfsearch( par::lf_search_snp );
      shutdown();
    }
