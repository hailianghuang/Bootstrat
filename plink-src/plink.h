...
public:
  ...
  bool operator== (const Range & b) const
  {
    return chr == b.chr && start == b.start && stop == b.stop && name == b.name;
  }
...
class Plink
{
 public: 
  ...
  // SNP attributes, generic string
  bool attribs;
  map<Locus*,string> attrib;
  ...
  void           readDataPooledFormat();
  void           readHaplotypeFormat();
  ...
  // RVTests
  void rTestWrapper();
  vector_t doRTests(bool,double,vector_t &,vector_t &,
		    vector<vector<RVData> >&,vector<int>&,ofstream&,ofstream&);
