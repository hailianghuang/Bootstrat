...
void Plink::readCNVList()
{
  ...
  if ( par::cnv_intersect )
      ...
      while ( ! IN.eof() )
	{
	  ...
	  if ( tokens.size() >= 5 )
	    {
	      map<string,int>::iterator i0 = Range::groupNames.find( tokens[4] );
	      if ( i0 != Range::groupNames.end() )
		{
		  r.group = i0->second;
		}
	      else
		{
		  r.group = Range::groupNames.size();
		  Range::groupNames.insert(make_pair( tokens[4] , r.group ) ); 
		}

	      // Append gene name, so we keep a second, unique copy
	      r.name += "_" + tokens[4];
...
vector_t Plink::glmCNVBurdenModel(Perm & perm, bool print )
{
  ...
  ////////////////////////////////////////////////
  // Generic covariates information
  ...
  for ( int i = 0; i < n; i++)
      ...
      if ( ic != segmentCount.end() )
	{
	  ...
	  // Filled in later:  sample[i]->clist[ testTerm ] = geneCount;
  ...
  ////////////////////////////////////////////////
  // Construct CNV gene counts
  
  if ( Range::groupNames.size() == 0 )
    {
      Range::groupNames.insert(make_pair("ALL",0));
    }

  map<int,string> pname;
  map<int,int> pord;
  map<int,int> pgcount; // how many genes per set
  map<int,int> counta;
  map<int,int> countu;

  map<int,set<string> > uniqa; // uniq genes counts
  map<int,set<string> > uniqu;

  static map<string,int>::iterator i = Range::groupNames.begin();
  while ( i != Range::groupNames.end() )
    {
      pname.insert(make_pair( i->second , i->first ));
      int tmp = pord.size();
      pord.insert(make_pair( i->second , tmp ));
  ...
  map<Individual*,int> indmap;
  for (int i=0; i<n; i++) 
    indmap.insert(make_pair( sample[i] , i ));
  
 
  int nGenes = geneList.size();
  int nPathway = pname.size();
  vector<vector<int> > pcount(n);
  
  for (int i=0; i<n; i++) 
    pcount[i].resize(nPathway,0);

  
  set<Range>::iterator i2 = geneList.begin();
  while ( i2 != geneList.end() )
    {
      map<int,int>::iterator igc = pgcount.find( i2->group );
      if ( igc == pgcount.end() )
	{
	  int tmp = i2->group;
	  pgcount.insert(make_pair( tmp , 1 ) );
	}
      else
	(igc->second)++;
      ++i2;
  ...
  //////////////////////////////////////////
  // Construct matrix of info.
 
  int c = 0;
  map<int,string>::iterator pi = pname.begin();
  while ( pi != pname.end() )
    {

      int slot = pord[ pi->first ];

      set<Range>::iterator i2 = geneList.begin();
      while ( i2 != geneList.end() )
	{

	  if ( pi->first != i2->group ) 
	    {
	      ++i2;
	      continue;
	    }
	  ...
	  while ( si != i->second.end() )
	    {
	      int idx = indmap[ si->p1 ];
	      pcount[idx][ slot ]++;

	      if ( sample[idx]->aff )
		{
		  map<int,int>::iterator ci1 = counta.find( slot );
		  if ( ci1 == counta.end() )
		    {
		      counta.insert(make_pair( slot , 1 ) );		     
		    }
		  else
		    (ci1->second)++;

		  map<int,set<string> >::iterator ui1 = uniqa.find( slot );
		  if ( ui1 == uniqa.end() )
		    {
		      set<string> tmp;
		      tmp.insert( tr.name );
		      uniqa.insert(make_pair( slot, tmp ));
		    }
		  else
		    ui1->second.insert( tr.name );
		} 
	      else // controls
		{
		  map<int,int>::iterator ci2 = countu.find( slot );
		  if ( ci2 == countu.end() )
		    {
		      countu.insert(make_pair( slot , 1 ) );
		    }
		  else
		    (ci2->second)++;
	
		  map<int,set<string> >::iterator ui1 = uniqu.find( slot );
		  if ( ui1 == uniqu.end() )
		    {
		      set<string> tmp;
		      tmp.insert( tr.name );
		      uniqu.insert(make_pair( slot, tmp ));
		    }
		  else
		    ui1->second.insert( tr.name );
...
//   map<int,string>::iterator p2 = pname.begin();
//   while ( p2 != pname.end() )
//     {
//       cout << p2->second << " " << p2->first << " " << pord[ p2->first ] << "\n";
//       int slot = pord[ p2->first ] ;
//       for (int i=0; i<n; i++) 
// 	cout << pcount[i][slot] << " ";
//       cout << "\n";
//       ++p2;
//     }
  ...
  while ( p2 != pname.end() )
    {
      ...
      int slot = pord[ p2->first ] ;
      string pwayname = p2->second;
      int ngenes = pgcount[ p2->first ];
      int na = counta[ slot ];
      int nu = countu[ slot ];
      int ua = uniqa[ slot ].size();
      int uu = uniqu[ slot ].size();
      ...
      for ( int i = 0; i < n; i++)
	sample[i]->clist[ testTerm ] = pcount[i][slot];
  ...
  if ( par::permute ) 
	...
	{      
	  ...
	  // Test each pathway for this replicated dataset
	  map<int,string>::iterator p2 = pname.begin();
	  while ( p2 != pname.end() )
	    {
